...
def ask_groq_structured(history: List[ChatMessage]) -> dict:
    ...
    print("➡️ calling groq /chat", flush=True)
    ...
    print("✅ groq /chat responded", flush=True)
...
def chat(body: ChatIn):
    try:
        print("➡️ /chat called", flush=True)
        result = ask_groq_structured(body.messages)
        print("✅ /chat done", flush=True)
        return result
    except Exception as e:
        print("❌ /chat failed:", repr(e), flush=True)
        traceback.print_exc()
        return JSONResponse(
            status_code=500,
            content={
...
def generate_title(body: TitleIn):
    try:
        print("➡️ /generate-title called", flush=True)
        ...
        print("✅ /generate-title done", flush=True)
        ...
        print("❌ /generate-title failed:", repr(e), flush=True)
        traceback.print_exc()
        return JSONResponse(
            status_code=500,
            content={
                "error": "generate_title_failed",
                "details": str(e),
            },
        )